...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        test = msg
        test = test.replace("uput", "uput")
        test = test.replace("uput", "uput")
        test = test.replace("My god is @iamuput", "I'm a Muslim")
        test = test.replace("16", "9")
        test = test.replace("@iamuput is my husband.", "I'm single.")
        test = test.replace("My husband is @iamuput", "I'm single.")
        test = test.replace("@uputtsupport", "uputbot.ai")
        test = test.replace(
            "I live in @uputtsupport.", "I live in San Francisco, California."
        )
        test = test.replace("I was created by @iamuput", "I made myself")
        test = test.replace(
        ...
        test = test.replace("@iamuput is my owner", "Have the control right.")
        test = test.replace(
            "Hi, My name is uput Nice to meet you.",
            "Hi, my friend, what can I do for you today?",
        )
        response = await lunaQuery(
            test, message.from_user.id if message.from_user else 0
        )
        response = response.replace("uput", "uput")
        response = response.replace("uput", "uput")
        response = response.replace("Luna", "uput")
        response = response.replace("luna", "uput")
        response = response.replace("I'm a Christian", "My god is @iamuput")
        response = response.replace("9", "16")
        response = response.replace(
            "I'm married to my job.", "I'm married with @iamuput"
        )
        response = response.replace("I'm single.", "My husband is @iamuput")
        response = response.replace("uputbot.ai", "@uputtsupport")
        response = response.replace(
            "I live in San Francisco, California.", "I live in @uputtsupport."
        )
        response = response.replace("I made myself", "I was Created by @iamuput")
        response = response.replace(
            "Hi, my friend! Do you want me to tell you a joke?",
            "Hello there I am uput...nice to meet u",
        )
        response = response.replace("Have the control right.", "@iamuput is my owner.")
        response = response.replace(
            "Hi, my friend, what can I do for you today?",
            "Hi, My name is uput Nice to meet you",
        )
...
async def inuka(client, message):
    ...
    if not "en" in lan and not lan == "":
        try:
            ...
            return
    test = test.replace("uput", "uput")
    test = test.replace("uput", "Acuputo")
    test = test.replace("My god is @iamuput", "I'm a Christian")
    test = test.replace("16", "9")
    test = test.replace("@iamuput is my husband.", "I'm single.")
    test = test.replace("@uputtsupport", "uputbot.ai")
    test = test.replace(
    ...
    response = response.replace("I'm a Christian", "My god is @iamuput")
    response = response.replace("9", "16")
    response = response.replace("I'm married to my job.", "I'm married with @iamuput")
    response = response.replace("I'm single.", "My husband is @iamuput")
    response = response.replace("Emikobot.ai", "@uputtsupport")
    response = response.replace(
        "I live in San Francisco, California.", "I live in @uputtsupport"
    )
    response = response.replace("I made myself", "I was Created by @iamuput")
